...
CTFAD S3 阶段 — 测试图像质量评估

用途：对放入 test_images/images/ 的图片进行全面质量分析，
...
输出：
    test_images/quality_report.json       ← 机器可读
    test_images/quality_report.html       ← 可视化报告（Safari 打开）
"""
...
# 路径配置（按需修改）
# ─────────────────────────────────────────────
BASE_DIR = Path("/Volumes/aiworkbench/projects/ctfad-pipeline/s3_experiments/test_images")
INPUT_DIR  = BASE_DIR / "images"          # 输入：原始图片
OUTPUT_DIR = BASE_DIR                     # 输出：报告放在 test_images/ 根目录
