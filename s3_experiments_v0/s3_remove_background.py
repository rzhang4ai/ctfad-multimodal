...
CTFAD S3 阶段 — 灵活去背景脚本（birefnet-general）
...
运行示例：
    ...
    # 方式2：处理手动列表（编辑下方 MANUAL_IMAGES）
    python s3_remove_background.py --mode manual

    # 方式3：命令行直接指定文件
    python s3_remove_background.py --mode files \
        --files /path/to/img1.jpg /path/to/img2.png

    # 不加参数，默认走 方式2（manual 模式）
    python s3_remove_background.py

输出：
    与输入图片同目录下的 images_rmbg_birefnet/ 子目录
...
MANUAL_IMAGES = [
    # ── A 级（质量评估推荐）──────────────────────────────────────────
    "cfaf_作品賞析_10251_250614_021蘇婉玲_作品_名片_特約教室_0935-895892.jpg",
    "cfaf_作品賞析_11240_250313_005黃美端.jpg",
    "ZHHY-ZR2.JPG",
    ...
    # ── 暂不处理（B 级）───────────────────────────────────────────────
    # "ZHHY-ZR1.JPG",        # 模糊(blur=56)、低对比度
    # "img_p0027_027.jpg",   # 严重模糊(blur=14)
    # "img_p0064_114.jpg",   # 过曝 + 分辨率过低
]
...
BASE_INPUT_DIR = Path(
    "/Volumes/aiworkbench/projects/ctfad-pipeline/s3_experiments/test_images/images"
)
